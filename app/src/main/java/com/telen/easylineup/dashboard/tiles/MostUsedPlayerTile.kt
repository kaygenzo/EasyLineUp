...
class MostUsedPlayerTile(context: Context) : ConstraintLayout(context) {
    ...
    fun bind(data: TileData, inEditMode: Boolean) {
        ...
        map.forEach { item ->
            when (item.key) {
                KEY_DATA_IMAGE -> binding.tilePlayerMostUsedImage.apply {
                    ready {
                        try {
                            Picasso.get()
                ...
                KEY_DATA_NAME -> binding.tilePlayerMostUsedName.text = item.value.toString().trim()
                KEY_DATA_SHIRT_NUMBER -> binding.tilePlayerMostUsedShirtNumber.text = item.value.toString()
            }
        }
